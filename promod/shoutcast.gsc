...
addPlayer()
{
	if(isDefined(self.pers["class"])&&!isDefined(self.pers["shoutnum"])&&(self.pers["team"]=="allies"||self.pers["team"]=="axis"))
	{
		...
		for(i=0;i<5;i++) if(!isDefined(level.shoutbars[i+offset]))
		{
...
removePlayer()
{
	if(isDefined(self.pers["shoutnum"]))
	{
		...
		for(i=0;i<5;i++)if(!isDefined(level.shoutbars[i]))
			for(j=i+1;j<5;j++) if(isDefined(level.shoutbars[j]))
			{
		...
		for(i=5;i<10;i++) if(!isDefined(level.shoutbars[i]))
			for(j=i+1;j<10;j++) if(isDefined(level.shoutbars[j]))
			{
				level.shoutbars[i]=level.shoutbars[j];
				level.shoutbars[i].pers["shoutnum"]=i;
				level.shoutbars[j]=undefined;
...
updatePlayer()
{
	for(i=0;i<level.players.size;i++)
		if(level.players[i].pers["team"]=="spectator"&&isDefined(self.pers["shoutnum"]))
			level.players[i]setClientDvars("shout_name"+self.pers["shoutnum"],self.name,"shout_health"+self.pers["shoutnum"],self.health/100);
}
...
loadOne()
{
	for(j=0;j<10;j++)
		if(isDefined(level.shoutbars[j]))
			self setClientDvars("shout_name"+j,level.shoutbars[j].name,"shout_health"+j,level.shoutbars[j].health/100);
		else 
			self setClientDvars("shout_name"+j,"","shout_health"+j,"");
}
