...
[[level.onSpawnPlayer]]();
// -Leiizko- //
self thread mickey\events::onSpawnPlayer();
// -Leiizko- //
	...
	if((level.roundLimit>1||(!level.roundLimit&&level.scoreLimit!=1))&&!level.forcedEnd)
	{
		...
		if( isDefined( level.caminfo ) && isDefined( level.caminfo[ team ] ) )
		{
			...
			if( level.leiizko_dvars[ "music" ] )
				level thread mickey\killcam::music();
			for(i=0;i<level.players.size;i++)
			{
				...
				level.players[i] thread mickey\killcam::killcam(  level.caminfo[ team ][ "attackerNum" ],
...
// -Leiizko- //
self thread mickey\events::onPlayerDisconnect();
// -Leiizko- //
lpselfnum = self getEntityNumber();
lpGuid = self getGuid();
...
{
	// -Leiizko- //
	self thread mickey\events::onPlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime );
	// -Leiizko- //
	...
	if(isDefined(sWeapon)&&isSubStr("m1014_mp winchester1200_mp mp5_mp uzi_mp ak74u_mp ak47_mp m14_mp mp44_mp g3_mp g36c_mp m16_mp m4_mp m40a3_mp remington700_mp m21_mp",sWeapon))self.attackerData[eAttacker.clientid]=true;
	}
...
Callback_PlayerKilled(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration)
{
	...
	/*if( getDvar( "g_gametype" ) == "killconfirmed" )
	{
		self thread mickey\killconfirmed::init(eInflictor,attacker,iDamage,sMeansOfDeath,sWeapon,vDir,sHitLoc,psOffsetTime,deathAnimDuration);
		break;
	}*/
	...
	// -Leiizko- //
	self thread mickey\events::onPlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
	// -Leiizko- //
	...
	{
		self mickey\killcam::killcam(lpattacknum,killcamentity,sWeapon,5,psOffsetTime,timeUntilRoundEnd(),attacker,self);
		/*wait 1;
