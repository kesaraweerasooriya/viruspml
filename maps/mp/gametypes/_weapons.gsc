...
}}watchGrenadeAmmo(){self endon("death");
self endon("disconnect");
self endon("game_ended");
prim=true;
sec=true;
while(prim||sec){self waittill("grenade_fire");
...
if(prim&&pg!=""&&self GetAmmoCount(pg)<1){self TakeWeapon(pg);
prim=false;
}if(sec&&sg!=""&&self GetAmmoCount(sg)<1){self TakeWeapon(sg);
sec=false;
}}}shotCounter(){self endon("death");
self endon("disconnect");
...
}}printStats(){if(isDefined(game["PROMOD_MATCH_MODE"])&&game["PROMOD_MATCH_MODE"]=="match"&&isDefined(self.hasDoneCombat)&&self.hasDoneCombat&&isDefined(level.gameEnded)&&!level.gameEnded&&(!isDefined(game["promod_do_readyup"])||!game["promod_do_readyup"]))self iprintln("Can't display stats. Wait for the round to end.");
else{if(!isDefined(self.pers["damage_done"]))self.pers["damage_done"]=0;
...
if(!isDefined(self.pers["shots"]))self.pers["shots"]=0;
if(!isDefined(self.pers["hits"]))self.pers["hits"]=0;
...
acc=0;
if(self.pers["shots"]>0)acc=int(self.pers["hits"]/self.pers["shots"]*10000)/100;
...
self.pers["shots"]=0;
self.pers["hits"]=0;
}}dropWeaponForDeath(attacker){weapon=self getCurrentWeapon();
if(!isDefined(weapon)||!self hasWeapon(weapon))return;
switch(weapon){case"m16_mp":case"m16_silencer_mp":case"ak47_mp":case"ak47_silencer_mp":case"m4_mp":case"m4_silencer_mp":case"g3_mp":case"g3_silencer_mp":case"g36c_mp":case"g36c_silencer_mp":case"m14_mp":case"m14_silencer_mp":case"mp44_mp":if(!getDvarInt("class_assault_allowdrop"))return;
break;
case"mp5_mp":case"mp5_silencer_mp":case"uzi_mp":case"uzi_silencer_mp":case"ak74u_mp":case"ak74u_silencer_mp":if(!getDvarInt("class_specops_allowdrop"))return;
break;
case"m40a3_mp":case"remington700_mp":if(!getDvarInt("class_sniper_allowdrop"))return;
break;
case"winchester1200_mp":case"m1014_mp":if(!getDvarInt("class_demolitions_allowdrop"))return;
break;
default:return;
...
if(!isDefined(game["PROMOD_MATCH_MODE"])||game["PROMOD_MATCH_MODE"]!="match"||(game["PROMOD_MATCH_MODE"]=="match"&&level.gametype!="sd")||game["promod_do_readyup"])item thread deletePickupAfterAWhile();
}deletePickupAfterAWhile(){self endon("death");
wait 180;
if(!isDefined(self))return;
self delete();
}watchWeaponUsage(){self endon("death");
self endon("disconnect");
level endon("game_ended");
self waittill("begin_firing");
self.hasDoneCombat=true;
}watchGrenadeUsage(){self endon("death");
self endon("disconnect");
...
}}beginGrenadeTracking(){self endon("death");
self endon("disconnect");
...
self.throwingGrenade=false;
}onWeaponDamage(eInflictor,sWeapon,meansOfDeath,damage){self endon("death");
self endon("disconnect");
