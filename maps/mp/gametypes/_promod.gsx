...
setClassChoice(classType)
{
	...
	switch(classType)
	{
		...
		case"specops":
			self setClientDvars("weap_allow_mp5",getDvar("weap_allow_mp5"),"weap_allow_uzi",getDvar("weap_allow_uzi"),"weap_allow_ak74u",getDvar("weap_allow_ak74u"),"attach_allow_specops_none",getDvar("attach_allow_specops_none"),"attach_allow_specops_silencer",getDvar("attach_allow_specops_silencer"), "weap_allow_skorpion", getDvar("weap_allow_skorpion"));
			break;
			
		case"demolitions":
			self setClientDvars("weap_allow_m1014",getDvar("weap_allow_m1014"),"weap_allow_winchester1200",getDvar("weap_allow_winchester1200"));
			break;
			
		case"sniper":
			self setClientDvars("weap_allow_m40a3",getDvar("weap_allow_m40a3"),"weap_allow_remington700",getDvar("weap_allow_remington700"), "weap_allow_m21", getDvar("weap_allow_m21"));
			break;
	}
...
else if(classType=="specops")loadout_primary=strTok("mp5,uzi,ak74u,skorpion",",");
else if(classType=="demolitions")loadout_primary=strTok("winchester1200,m1014",",");
else if(classType=="sniper")loadout_primary=strTok("m40a3,remington700,m21",",");
...
switch(type)
{
case"loadout_primary":for(i=0;
i<loadout_primary.size;
i++)
{
...
break;
case"loadout_primary_attachment":if(classType=="assault"||classType=="specops")
{
for(i=0;
i<loadout_primary_attachment.size;
i++)
{
...
break;
case"loadout_secondary":for(i=0;
i<loadout_secondary.size;
i++)
{
...
break;
case"loadout_secondary_attachment":for(i=0;
i<loadout_secondary_attachment.size;
i++)
{
...
break;
case"loadout_grenade":for(i=0;
i<loadout_grenade.size;
i++)
{
...
switch(subTokens[0])
{
case"loadout_primary":case"loadout_secondary":if(getDvarInt("weap_allow_"+subTokens[1])&&self verifyWeaponChoice(subTokens[1],self.class))
{
...
break;
case"loadout_primary_attachment":case"loadout_secondary_attachment":if((subTokens[1]=="assault"||subTokens[1]=="specops"||subTokens[1]=="pistol")&&(subTokens[2]=="none"||subTokens[2]=="silencer"))
{
...
case"loadout_grenade":switch(subTokens[1])
{
case"flash_grenade":case"smoke_grenade":if(getDvarInt("weap_allow_"+subTokens[1]))
{
self.pers[self.class][subTokens[0]]=subTokens[1];
self setClientDvar(subTokens[0],subTokens[1]);
}
...
{
case"camo_none":case"camo_brockhaurd":case"camo_bushdweller":case"camo_blackwhitemarpat":case"camo_tigerred":case"camo_stagger":case"camo_gold":self.pers[self.class][subTokens[0]]=subTokens[1];
break;
default:return;
}
...
switch(classType)
{
case"assault":case"sniper":if(tableLookup("mp/statsTable.csv",4,weaponName,2)=="weapon_"+classType)return true;
break;
case"specops":if(tableLookup("mp/statsTable.csv",4,weaponName,2)=="weapon_smg")return true;
break;
case"demolitions":if(tableLookup("mp/statsTable.csv",4,weaponName,2)=="weapon_shotgun")return true;
break;
}
...
verifyClassChoice(teamName,classType)
{
	...
	{
		if( isDefined( self ) && isDefined( self.pers[ "VIP" ] ) && self.pers[ "VIP" ] && self.pers[ "VIP_STAGE" ] >= 2 )
			return true;
